...
> **安装**: `uv tool install xiaohongshu-cli`，然后 `xhs login`（自动从浏览器提取 Cookie）。
>
...
>
> **POST 操作风险**: 发帖(post)、评论(comment)、点赞(like) 等写操作在 v0.6.x 可能因签名问题返回 406。如需使用，建议降级到 v0.3.5 (`uv tool install --force 'xiaohongshu-cli==0.3.5'`)。
...
# 如果 search 返回 404，升级 twitter-cli：uv tool upgrade twitter-cli
...
> **安装**: `uv tool install twitter-cli`（确保 v0.8.5+）
>
...
>
> **search 可能失效**: Twitter 频繁修改 GraphQL API，search 命令可能随时返回 404。如遇到，先 `uv tool upgrade twitter-cli`。如果最新版仍不行，说明上游还没跟上 Twitter 的改动，用 `twitter feed` 替代。
>
...
> **安装**: `uv tool install 'git+https://github.com/public-clis/rdt-cli.git'`（PyPI 版本暂时落后，需从 GitHub 装 v0.4.2+）。需要先登录（`rdt login`）才能搜索和阅读。
